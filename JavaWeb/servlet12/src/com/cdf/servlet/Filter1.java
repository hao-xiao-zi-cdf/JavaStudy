...
public class Filter1 implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        System.out.println("Filter1的init()方法执行...");
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        System.out.println("Filter1对servlet程序执行前进行过滤...");
        //跳转到下一个过滤器，没有过滤器则执行servlet程序
        chain.doFilter(request,response);
        //在servlet程序执行后，对响应进行过滤
        System.out.println("Filter1对servlet程序执行后进行过滤...");
    }

    //destroy()方法会在Filter对象释放前执行
    @Override
    public void destroy() {
        System.out.println("Filter1的destroy()方法执行...");
    }
}
